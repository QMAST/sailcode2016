        ...
        pchamp_servo rudder[2] )
{
	...
	if(rc_get_mapped_analog(rc->lsx, -1000, 1000) == -3750){ //rc not connected
		// Set all motors to zero
		Serial.println("RC not connected... setting all motors to zero.");
		motor_set_rudder(0);
		motor_set_winch(0);
		SERIAL_PORT_KEEL_SW.write(KEEL_POSE_CENTER);
	}
	else{//normal operation
		Serial.println("RC connected!  Switching to RC control...");
